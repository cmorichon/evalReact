...
const Galerie = () => {
  ...
  const {data} = useQuery('contentfulEntriesGalerie', fetchContentfulEntriesGalerie,{
    onSuccess: (data) =>{
      ...
      data.map(image => {
        ...
        return setDataGalerie((prevDataGalerie)=>[...prevDataGalerie, imageInfo]);
    })    
    }
  } )
  ...
  const filterImage = () =>{
    if(activeTag ){
      return (
      dataGalerie.filter(dataGalerieFilter => dataGalerieFilter.tag.includes(activeTag)).map(elt =>(
        <ImageCard
            tag={elt.tag}
            key={elt.index}
            alt={elt.name}
  ...
  <section className='container m-auto '>
    <Title
    content='La galerie photo'
    />
    <Button 
      onClick={(e)=> setActiveTag()} 
      content ='Tous'
    />
    {
    tags ? 
